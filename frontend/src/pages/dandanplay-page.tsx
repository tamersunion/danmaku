...
import type {
  ...
  DandanplayKeyword,
  DandanplayPool,
  DandanplayPoolDanmaku,
  ManagedVideo,
} from "@/api/types";
import { ConfirmAction } from "@/components/confirm-action";
import { DandanplaySearch } from "@/components/dandanplay-search";
...
type PoolMutationResult = { pool: DandanplayPool; inserted: number };

export function DandanplayPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        eyebrow="第三方弹幕库"
        title="弹弹play"
        description="持久化管理弹弹play弹幕池、视频关联和过滤规则"
      />
...
function PoolPanel() {
  ...
  const creationBinding = usePoolVideoBinding<ApiResponse<PoolMutationResult>>("dandanplay", createOpen, (result) => result.data.pool.id);
  ...
  const pools = useQuery({
    queryKey: ["dandanplay-pools", page, query],
    queryFn: async () =>
      (
        await apiGet<ApiResponse<Paged<DandanplayPool>>>("/api/admin/dandanplay/pools", {
          page,
  ...
  >({
    mutationFn: (body) => creationBinding.create(body, () => apiPost<ApiResponse<PoolMutationResult>>("/api/admin/dandanplay/pools", body)),
    successMessage: creationBinding.videoID ? "弹幕池已创建、同步并关联视频" : "弹幕池已创建并同步",
    invalidate: [["videos"], ["video"], ["video-heatmap"], ["dandanplay-pools"], ["dandanplay-pool-options"]],
  });
  const sync = useApiMutation<number, ApiResponse<PoolMutationResult>>({
    mutationFn: (id) => apiPost(`/api/admin/dandanplay/pools/${id}/sync`),
    successMessage: "弹幕池已增量同步",
    invalidate: [
      ["videos"], ["video"], ["video-heatmap"],
      ["dandanplay-pools"],
      ["dandanplay-pool-options"],
      ["dandanplay-pool-danmaku"],
    ],
  });
  const columns = useMemo<DataColumn<DandanplayPool>[]>(
    () => [
      {
        key: "pool",
        label: "弹幕池",
    ...
    <>
          ...
          >
            <FieldGroup>
              <Field orientation="horizontal">
                <Input
                  aria-label="搜索弹弹play 剧集 ID"
                  value={draftQuery}
                  placeholder="搜索弹弹play 剧集 ID"
      ...
      <Dialog open={createOpen} onOpenChange={(open) => { if (!create.isPending) setCreateOpen(open); }}>
        <DialogContent className="max-h-[calc(100svh-2rem)] overflow-y-auto sm:max-w-xl">
          ...
          >
            <DialogHeader>
              <DialogTitle>添加弹弹play弹幕池</DialogTitle>
              <DialogDescription>
                搜索作品选择剧集，或手动输入剧集 ID；创建时立即获取弹幕并建立本地缓存
              </DialogDescription>
            </DialogHeader>
            {createOpen ? <DandanplaySearch episodeId={episodeId} onSelect={setEpisodeID} disabled={create.isPending} /> : null}
            <FieldGroup>
            <Field data-disabled={create.isPending}>
              <FieldLabel htmlFor="dandanplay-pool-episodeId">弹弹play 剧集 ID</FieldLabel>
              <Input
                id="dandanplay-pool-episodeId"
...
function BindingDialog({ pool, onOpenChange }: { pool: DandanplayPool | null; onOpenChange: (open: boolean) => void }) {
  const [videoID, setVideoID] = useState("");
  const [offset, setOffset] = useState("0");
  const videos = useQuery({
    queryKey: ["dandanplay-binding-video-options"],
    queryFn: async () =>
      (
  ...
  const bind = useApiMutation<
    ...
    ApiResponse<DandanplayBinding>
  >({
    mutationFn: ({ videoID: targetVideoID, ...body }) =>
      apiPost(`/api/admin/videos/${targetVideoID}/dandanplay-bindings`, body),
    successMessage: "弹幕池已关联到视频",
    invalidate: [["videos"], ["video"], ["video-heatmap"], ["videos"], ["video"], ["dandanplay-pools"]],
  });
      ...
      <DialogContent>
        <form className="flex flex-col gap-5" onSubmit={submit}>
          ...
          {videos.isError ? (
            ...
            <FieldGroup>
              <Field>
                <FieldLabel htmlFor="dandanplay-binding-video">视频</FieldLabel>
                <SearchableSelect
                  id="dandanplay-binding-video"
              ...
              <Field>
                <FieldLabel htmlFor="dandanplay-binding-offset">偏移量（秒）</FieldLabel>
                <Input id="dandanplay-binding-offset" type="number" step="any" value={offset} required onChange={(event) => setOffset(event.target.value)} />
              </Field>
            </FieldGroup>
          )}
          <DialogFooter>
...
function PoolDanmakuDialog({ pool, onOpenChange }: { pool: DandanplayPool | null; onOpenChange: (open: boolean) => void }) {
  const [page, setPage] = useState(1);
  const [draftQuery, setDraftQuery] = useState("");
  const [query, setQuery] = useState("");
  const [blocked, setBlocked] = useState("all");
  const danmaku = useQuery({
    queryKey: ["dandanplay-pool-danmaku", pool?.id, page, query, blocked],
    queryFn: async () =>
      (
        await apiGet<ApiResponse<Paged<DandanplayPoolDanmaku>>>(
          `/api/admin/dandanplay/pools/${pool?.id}/danmaku`,
  ...
  >({
    mutationFn: ({ id, blocked: value }) =>
      apiPatch(`/api/admin/dandanplay/danmaku/${id}/blocked`, { blocked: value }),
    successMessage: "屏蔽状态已更新",
    invalidate: [["videos"], ["video"], ["video-heatmap"], ["dandanplay-pool-danmaku"], ["dandanplay-pools"]],
  });
  const columns: DataColumn<DandanplayPoolDanmaku>[] = [
    {
      key: "content",
      label: "弹幕内容",
...
function KeywordPanel({ pools }: { pools: DandanplayPool[] }) {
  const [scope, setScope] = useState("global");
  const [keyword, setKeyword] = useState("");
  const keywords = useQuery({
    queryKey: ["dandanplay-keywords"],
    queryFn: async () => (await apiGet<ApiResponse<DandanplayKeyword[]>>("/api/admin/dandanplay/keywords")).data,
  });
  const create = useApiMutation<
    { poolId: number | null; keyword: string },
    ApiResponse<DandanplayKeyword>
  >({
    mutationFn: (body) => apiPost("/api/admin/dandanplay/keywords", body),
    successMessage: "关键词过滤规则已添加",
    invalidate: [["videos"], ["video"], ["video-heatmap"], ["dandanplay-keywords"], ["dandanplay-pools"], ["dandanplay-pool-danmaku"]],
  });
  const remove = useApiMutation<number, ApiResponse<null>>({
    mutationFn: (id) => apiDelete(`/api/admin/dandanplay/keywords/${id}`),
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>全局规则作用于所有弹弹play弹幕池；池级规则只作用于指定剧集 ID；弹幕数据仍会完整保留</CardDescription>
      </CardHeader>
      <CardContent>
        <form
        ...
        >
          <FieldGroup className="grid gap-4 md:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
            <Field>
              <FieldLabel htmlFor="dandanplay-keyword-scope">作用范围</FieldLabel>
              <SearchableSelect
            ...
            <Field>
              <FieldLabel htmlFor="dandanplay-keyword-value">关键词</FieldLabel>
              <div className="flex gap-2">
                <Input id="dandanplay-keyword-value" value={keyword} maxLength={200} required onChange={(event) => setKeyword(event.target.value)} />
