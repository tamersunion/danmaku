...
export function DandanplaySearch({ episodeId, onSelect, disabled = false }: {
  episodeId: string;
  ...
  const search = useQuery({
    queryKey: ["dandanplay-search", keyword],
    queryFn: async () => (await apiGet<ApiResponse<Anime[]>>("/api/admin/dandanplay/search", { keyword })).data,
  ...
  const episodes = useQuery({
    queryKey: ["dandanplay-episodes", animeId],
    queryFn: async () => (await apiGet<ApiResponse<Episode[]>>(`/api/admin/dandanplay/anime/${animeId}/episodes`)).data,
  ...
  return (
    <FieldGroup>
      <Field data-disabled={disabled}>
        <FieldLabel htmlFor="dandanplay-search-keyword">关键词搜索</FieldLabel>
        <div className="flex gap-2">
          <Input
            id="dandanplay-search-keyword"
            value={draft}
            disabled={disabled}
            maxLength={200}
            placeholder="输入作品名称"
      ...
      {keyword && search.isSuccess && !search.isFetching ? (
        <Field data-disabled={disabled || !search.data.length}>
          <FieldLabel htmlFor="dandanplay-search-anime">作品</FieldLabel>
          <SearchableSelect
            id="dandanplay-search-anime"
      ...
      {animeId ? (
        <Field data-disabled={disabled || episodes.isFetching || !episodes.data?.length}>
          <FieldLabel htmlFor="dandanplay-search-episode">剧集</FieldLabel>
          <SearchableSelect
            id="dandanplay-search-episode"
