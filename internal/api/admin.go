...
func (s *Server) serveAdmin(w http.ResponseWriter, r *http.Request, path string, current session) {
	...
	switch {
	case catalogAdminSource(path) != "":
		s.serveCatalogAdmin(w, r, path, s.catalogs[catalogAdminSource(path)])
	...
	case path == "/api/admin/animeko" || strings.HasPrefix(path, "/api/admin/animeko/"):
		s.serveAnimekoAdmin(w, r, path)
	case path == "/api/admin/dandanplay" || strings.HasPrefix(path, "/api/admin/dandanplay/"):
		s.serveDandanplayAdmin(w, r, path)
