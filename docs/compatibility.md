...
2.9.0 在弹幕管理中增加关键词过滤、用户名映射和 IP 黑名单。启动事务自动创建 `NativeDanmakuRule` 表与唯一约束，无需手工 SQL。新增管理路由为 `/api/admin/danmaku-rules/{keywords|authors|ips}`（GET 列表、POST 创建）及其 `/{id}`（DELETE 删除）；管理员和弹幕管理员可操作，普通用户不可访问。

- 关键词按不区分大小写的子串匹配，新提交命中时仍录入但置为已删除，所有发送接口仍返回 `{"code":0,"data":null}`。不追溯历史记录，删除规则不会自动恢复已删除记录；原有 30 秒同 IP、VID、内容重复丢弃规则不变。
- 用户名按对外显示的作者名精确匹配、区分大小写，只应用一次，不递归映射。未设置作者名时以原 `authorId` 的十进制文本匹配。新数据保存替换后的 `Author`，不修改平台用户、作者 ID 或第三方弹幕。创建时 `scanExisting` 默认 false；开启后在同一事务内替换系统全库匹配记录（包括软删除记录），返回 `replaced` 条数并刷新系统池／合并池缓存。此替换不可通过删除规则撤销；全库扫描期间新提交会等待事务完成。
- IP 支持单个 IPv4／IPv6 或 CIDR，网段会规范化，IPv4 映射 IPv6 地址按 IPv4 判定。命中黑名单优先拒绝，不创建视频或弹幕，即使命中关键词或重复提交，也只返回 `{"code":1,"data":null}`，不暴露原因。
- IP 来源依次为可信代理覆盖的 `X-Real-IP`、`X-Forwarded-For`、连接地址，仅缺少有效连接地址时才回退到请求体。容器必须置于会覆盖客户端 IP 头的可信代理后，禁止客户端绕过代理直接访问；当前 OpenResty 源站配置覆盖 `X-Real-Ip`。旧的响应 `sub_filter` 用户名替换应移除，否则会干扰规则管理和输出，原硬编码名称不自动预置为新规则。

可设置 `DANMAKU_TEST_POSTGRES` 为专用测试数据库连接串，运行 `go test ./internal/store -run TestNativeRulePostgresIntegration -v`，验证迁移重复执行、关键词软删除、历史替换、缓存失效及黑名单拒绝。测试仅在随机独立 schema 内建表，结束后清理该 schema；未提供该变量时跳过，不自动连接部署数据库。
